...
$allowed_to_save = (
                        !$data->pic_signed_by || 
                        (
                            $data->pic_signed_by == sessionData('user_id') || 
                            $data->approver_id == sessionData('user_id')
                        )
                    ) &&
                ...
                <div class="col-md-12 row">
                    ...
                    <div class="col-md-3">
                        <label class="form-label">Tanda tangan Koor. Editor</label>
                            ...
                            <div>
                                <div style="<?=$data->is_send && $data->approver_id == sessionData('user_id') && !$data->approver_signature ? '' : 'display:none'?>">
...
<script>
    ...
    $(document).ready(function() {
        ...
        saveOrSend = (isSend = false) => {
            ...
            $.ajax({
                url: "<?=site_url('naskah/save_sop_editing')?>",
                data: data,
                method: "POST",
                success: function () {
                    Swal.fire(
                        'Berhasil menyimpan!',
                        'SOP Editing berhasil disimpan..',
                        'success'
                    ).then(function() {
